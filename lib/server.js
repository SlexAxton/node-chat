...
var handlers = [
	...
	{ path: "/join", handler: function(channel, request, response) {
		sys.puts("Request URL: " + request.url);
		...
		var session = channel.createSession(nick, options);
		if (!session) {
			response.simpleJSON(400, { error: "nick in use." });
			return;
		}
		
		response.simpleJSON(200, { id: session.id, nick: nick, options: options, since: session.since });
	} },
